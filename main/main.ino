...
void updateNavigationDisplay()
{
  ...
  showText(0, line, currentNavData.title + " nữa " + currentNavData.directions, ST77XX_WHITE);
  showText(0, line, currentNavData.eta, ST77XX_WHITE);
  showText(0, line, "Speed: " + currentNavData.speed, ST77XX_WHITE);
}
...
void myRingerHandler(String caller, bool incoming)
{
  ...
  if (incoming)
  {
    ...
    priority = 0x03 showText(0, line, F("Cuộc gọi đến từ: "), ST77XX_WHITE);
...
void UpdateDisplay()
{
  StateDisplay displayState = 0;
  if ((priority & MASK_CALL) != 0)
  {

    displayState = (priority & 1) == 1 ? CALLING : ENDCALL;
  }
